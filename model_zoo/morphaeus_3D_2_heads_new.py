...
class MorphAEus(nn.Module):
    ...
    def __init__(self,
        ...
        self.deformer_b_1 = Deformer(inshape=inshape, in_channels=in_channels, channels=ref_channels,
                              strides=strides, kernel_size=kernel_size, norm=norm, act=act, deconv_mode=deconv_mode,
                                name_prefix='ref_', bidir=bidir)
        self.deformer_b_01 = Deformer(inshape=inshape, in_channels=in_channels, channels=ref_channels,
                              strides=strides, kernel_size=kernel_size, norm=norm, act=act, deconv_mode=deconv_mode,
                                name_prefix='ref_', bidir=bidir)       
    ...
    def forward(self, x, registration=False):
     ...
     #   y_source_b1, y_target_b1, preint_flow_b1, pos_flow_b1 = self.deformer_b_1(x, dec_x, encode_history, decode_history, registration)
      #  y_source_b01, y_target_b01, preint_flow_b01, pos_flow_b01 = self.deformer_b_01(x, dec_x, encode_history, decode_history, registration)
        # return non-integrated flow field if training
        if not registration:
            return y_source, {'deformation': preint_flow, 'x_prior': dec_x, 'x_reversed': y_target, 'encode_history': encode_history,'decode_history':decode_history}
         #          y_source_b1, {'deformation': preint_flow_b1, 'x_prior': dec_x, 'x_reversed': y_target_b1, 'embeddings': encode_history},\
          #         y_source_b01, {'deformation': preint_flow_b01, 'x_prior': dec_x, 'x_reversed': y_target_b01, 'embeddings': encode_history}
           # return  {'x_prior': dec_x, 'embeddings': encode_history}
        else:
            return y_source, {'deformation': pos_flow, 'x_prior': dec_x, 'x_reversed': y_source, 'encode_history': encode_history}
          #  return  {'x_prior': dec_x, 'embeddings': encode_history}

    def forward_b1(self,x,dec_x,encode_history,decode_history,registration):
        y_source, y_target, preint_flow, pos_flow = self.deformer_b_1(x, dec_x, encode_history, decode_history, registration)
        return y_source, {'deformation': preint_flow, 'x_reversed': y_target}

    def forward_b01(self,x,dec_x,encode_history,decode_history,registration):
        y_source, y_target, preint_flow, pos_flow = self.deformer_b_01(x, dec_x, encode_history, decode_history, registration)
        return y_source, {'deformation': preint_flow, 'x_reversed': y_target}