...
class Deformer(nn.Module):
    def __init__(self, inshape, channels: Sequence[int], strides: Sequence[int], in_channels=1,
            ...
            if deconv_mode == 'stride_upsample' and i!=0:
                ...
                self.deformerNN.add_module(name_prefix + "_refine_%i" % i,
                                           nn.ConvTranspose3d(in_channels=layer_channels, out_channels=32,
                                                   kernel_size=(kernel_size, kernel_size,kernel_size), stride=s,
                                                   padding=padding,
                                                   output_padding=1))
            if norm == 'batch':
                self.deformerNN.add_module(name_prefix + "_batch_%i" % i, nn.BatchNorm3d(32))
        ...
        self.flow = nn.Conv3d(32, ndims, kernel_size=3, padding=1)
    ...
    def forward(self, x, dec_x, encode_history, decode_history, registration):
        ...
        for i_r, ref_layer in enumerate(self.deformerNN):
            if i_r == 0:
                ...
                if isinstance(ref_layer, nn.Conv3d) or isinstance(ref_layer, nn.ConvTranspose3d):
