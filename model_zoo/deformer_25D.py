...
class Deformer(nn.Module):
    def __init__(self, inshape, channels: Sequence[int], strides: Sequence[int], in_channels=1,
        ...
        self.flow = nn.Conv2d(32, ndims*3, kernel_size=3, padding=1)
        self.flow.weight = nn.Parameter(Normal(0, 2e-2).sample(self.flow.weight.shape))
    ...
    def forward(self, x, dec_x, encode_history, decode_history, registration):
        ...
        for i in range(3):
         # with torch.no_grad():    
            ...
            y_source = self.transformer(dec_x[:,i,np.newaxis,:,:], pos_flow)
            y_target = self.transformer(x[:,i,np.newaxis,:,:], neg_flow) if self.bidir else y_source
            #
            if i ==0:
                y_source_all=y_source.cpu().detach().numpy()
                y_target_all=y_target.cpu().detach().numpy()
                preint_flow_all=preint_flow.cpu().detach().numpy()
                pos_flow_all=pos_flow.cpu().detach().numpy()
            else:
                y_source_all=np.concatenate((y_source_all,y_source.cpu().detach().numpy()),axis=1)
                y_target_all=np.concatenate((y_target_all,y_target.cpu().detach().numpy()),axis=1)
                preint_flow_all=np.concatenate((preint_flow_all,preint_flow.cpu().detach().numpy()),axis=1)
                pos_flow_all=np.concatenate((pos_flow_all,pos_flow.cpu().detach().numpy()),axis=1)

        return y_source_all, y_target_all, preint_flow_all, pos_flow_all
