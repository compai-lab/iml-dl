...
#                        'optimizer_weights_b1': self.optimizer_deformer_b1.state_dict(),
           ...
           # deff=rec_
            ...
            if epoch > 10:
                ...
                reconstruction_b1_ = reconstruction_b1[0].cpu().detach().numpy()
                reconstruction_b01_ = reconstruction_b01[0].cpu().detach().numpy()
                elements = [img,gl_prior, rec_,reconstruction_b1_,reconstruction_b01_,np.abs(gl_prior-img), np.abs(rec_ - img),jacdet,deff]
